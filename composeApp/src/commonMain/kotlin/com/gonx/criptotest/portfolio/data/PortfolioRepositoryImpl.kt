...
import com.gonx.criptotest.core.domain.onError
import com.gonx.criptotest.core.domain.onSuccess
...
import com.gonx.criptotest.portfolio.data.mapper.toPortfolioCoinEntity
import com.gonx.criptotest.portfolio.data.mapper.toPortfolioCoinModel
...
class PortfolioRepositoryImpl(
    ...
    override suspend fun initializeBalance() {
        ...
        if (currentBalance == null) userBalanceDao.insertBalance(UserBalanceEntity(cashBalance = 10000.0))
    }
                        ...
                        .onSuccess { coinsDto ->
                            val portfolioCoins =
                                portfolioCoinEntities.mapNotNull { portfolioCoinEntity ->
                                    val coin =
                                        coinsDto.data.coins.firstOrNull { it.uuid == portfolioCoinEntity.coinId }
                                    coin?.let {
                    ...
                    apiResult
                        ...
                        .onSuccess { coinsDto ->
                            val totalValue = portfolioCoinEntities.sumOf { ownedCoin ->
                                val coinPrice =
                                    coinsDto.data.coins.firstOrNull { it.uuid == ownedCoin.coinId }?.price
                                        ?: 0.0
    ...
    override fun cashBalance(): Flow<Double> {
        return flow { emit(userBalanceDao.getCashBalance() ?: 10000.0) }
    }
